            ...
            <main className="relative z-10 w-full max-w-lg px-4 py-12">
                <div className="bg-white/95 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl p-8 md:p-10">
                    <div className="text-center mb-8">
                        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">تسجيل دخول الإدارة</h1>
                        <p className="text-gray-500">مخصص لمديري منصة {t.appName}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1">
                                ...
                                <Link href="/forgot-password" className="text-xs text-[#D4A843] hover:underline">{t.auth.forgotPassword}</Link>
                            </div>
                            <div className="relative">
                                <Lock className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                                <input id="password" type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="........" dir="ltr" className="w-full pr-10 pl-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#0B3D2E] focus:border-[#0B3D2E] transition-colors text-sm" required />
                        ...
                        <button type="submit" disabled={loading} className="w-full bg-[#0B3D2E] hover:bg-[#072A1F] text-white font-bold py-3.5 px-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 flex items-center justify-center gap-2 disabled:opacity-60">
                            {loading ? (
