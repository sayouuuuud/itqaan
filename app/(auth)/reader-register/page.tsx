      ...
      <main className="relative z-10 w-full max-w-2xl px-4 py-6 pb-16">
        <div className="bg-card/95 backdrop-blur-xl border border-border shadow-2xl rounded-2xl p-8 md:p-10">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-foreground mb-2">{t.readerRegister.title}</h1>
            <p className="text-muted-foreground">{t.readerRegister.desc}</p>
          </div>
                    ...
                    <Popover open={nationalityOpen} onOpenChange={setNationalityOpen}>
                      <PopoverTrigger asChild>
                        <button
                          type="button"
                          className={cn(
                            "w-full pr-4 pl-10 py-3 bg-secondary/20 dark:bg-secondary/10 border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-sm flex items-center justify-between",
          ...
          <div className="mt-8 text-center border-t border-gray-100 pt-6">
            <p className="text-sm text-gray-600 mb-2">
              {t.locale === 'ar' ? 'تريد التسجيل كطالب لحفظ القرآن؟' : 'Want to register as a student to memorize the Quran?'}
            </p>
            <Link href="/register" className="inline-flex items-center justify-center text-sm font-bold text-[#0B3D2E] hover:text-[#0B3D2E]/80 transition-colors">
              {t.locale === 'ar' ? 'سجل كطالب من هنا' : 'Register as a Student here'}
            </Link>
          </div>
        </div>
      </main>
