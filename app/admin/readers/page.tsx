...
export default function AdminReadersPage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-900">{isAr ? 'إدارة المقرئين' : 'Readers Management'}</h1>
                <p className="text-sm text-gray-500 mt-1">{isAr ? 'عرض وتعديل بيانات المقرئين المعتمدين' : 'View and edit approved reader profiles'}</p>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <Input className="pr-10 border-gray-200" placeholder={isAr ? 'بحث باسم أو بريد...' : 'Search by name or email...'} value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} />
                </div>
                <select
                ...
                >
                    <option value="">{isAr ? 'كل الجنسين' : 'All Genders'}</option>
                    <option value="male">{isAr ? 'ذكور' : 'Male'}</option>
                    <option value="female">{isAr ? 'إناث' : 'Female'}</option>
                </select>
            </div>
                        ...
                        <div key={r.id} className={`bg-white border rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow ${!r.is_active ? 'opacity-60 border-red-200' : 'border-gray-100'}`}>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${r.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                                {r.is_active ? (isAr ? 'نشط' : 'Active') : (isAr ? 'معطل' : 'Inactive')}
                                            </span>
                                            <span className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">
                                                {r.gender === 'male' ? (isAr ? 'ذكر' : 'Male') : r.gender === 'female' ? (isAr ? 'أنثى' : 'Female') : ''}
                                            </span>
                                        </div>
                            ...
                            <div className="mt-4 grid grid-cols-3 gap-3 pt-4 border-t border-gray-100">
                                <div className="text-center">
                                    ...
                                    <p className="text-[10px] text-gray-500">{isAr ? 'مراجعات' : 'Reviews'}</p>
                                </div>
                                <div className="text-center">
                                    <p className="text-lg font-bold text-gray-900">{r.sessions_done || 0}</p>
                                    <p className="text-[10px] text-gray-500">{isAr ? 'جلسات' : 'Sessions'}</p>
                                </div>
                                <div className="text-center">
                                    ...
                                    <p className="text-[10px] text-gray-500">{isAr ? 'التقييم' : 'Rating'}</p>
                                </div>
                            </div>
            ...
            {readers.length === 0 && !loading && (
                <div className="text-center py-16 text-muted-foreground">
                    ...
                    <p>لا يوجد مقرئون معتمدون بعد</p>
                </div>
            )}
            ...
            <Dialog open={!!editReader} onOpenChange={() => setEditReader(null)}>
                <DialogContent className="max-w-lg">
                    <DialogHeader>
                        <DialogTitle>تعديل بيانات المقرئ</DialogTitle>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setEditReader(null)}>إلغاء</Button>
