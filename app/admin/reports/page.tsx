...
import { BarChart3, TrendingUp, Download, Users, Star, Mail, Award, Loader2, Medal, Clock, MapPin, AlertCircle } from 'lucide-react'
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: t.admin.reportsPage.totalSessions, value: data?.sessions?.total || 0, icon: Users, color: 'text-purple-600', bg: 'bg-purple-50' },
          { label: t.admin.reportsPage.completedSessions, value: data?.sessions?.completed || 0, icon: Clock, color: 'text-teal-600', bg: 'bg-teal-50' },
          { label: t.admin.reportsPage.needsSession, value: data?.recitations?.needs_session || 0, icon: Star, color: 'text-orange-600', bg: 'bg-orange-50' },
          { label: t.admin.reportsPage.certificatesIssued, value: data?.certificates || 0, icon: Award, color: 'text-yellow-600', bg: 'bg-yellow-50' },
          { label: t.admin.reportsPage.emailsSent, value: data?.emailsSent || 0, icon: Mail, color: 'text-pink-600', bg: 'bg-pink-50' },
        ].map(card => (
      ...
      {/* Errors Summary Row */}
      {data?.wordMistakes?.topWords?.length > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-4">{t.admin.reportsPage.monthlyTrend}</h3>
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        {/* Weekly Mastery % */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-4">{t.admin.reportsPage.weeklyMasteryRate}</h3>
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data?.masteryTrend || []}>
                ...
                <Tooltip formatter={(v: any) => [`${v}%`, t.admin.reportsPage.masteryRate]} contentStyle={{ borderRadius: '12px', border: 'none' }} />
                <Bar dataKey="mastery_rate" fill="#10b981" radius={[4, 4, 0, 0]} name={t.admin.reportsPage.masteryRate} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Gender dist */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-4 text-sm">{t.admin.reportsPage.genderStudents}</h3>
              ...
              return (
                <div key={g.gender} className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-500 font-medium">{g.gender === 'male' ? t.auth.male : g.gender === 'female' ? t.auth.female : t.auth.unspecified}</span>
        ...
        {/* Session details */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-4 text-sm">{t.admin.reportsPage.sessionsDetails}</h3>
          <div className="space-y-3">
            {[
              { label: t.admin.reportsPage.completed, value: data?.sessions?.completed || 0, color: 'text-green-600', bg: 'bg-green-50' },
              { label: t.admin.reportsPage.cancelled, value: data?.sessions?.cancelled || 0, color: 'text-red-600', bg: 'bg-red-50' },
              { label: t.admin.reportsPage.noShow, value: data?.sessions?.no_show || 0, color: 'text-gray-600', bg: 'bg-gray-100' },
              { label: t.admin.reportsPage.avgDuration, value: `${data?.sessions?.avg_duration || 0} ${t.admin.reportsPage.min}`, color: 'text-blue-600', bg: 'bg-blue-50' }
            ].map(s => (
        ...
        {/* Top cities */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-4 text-sm">{t.admin.reportsPage.topCities}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-gray-800">{t.admin.reportsPage.topReviewers}</h3>
          </div>
          <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-gray-800">{t.admin.reportsPage.topSessionReaders}</h3>
          </div>
          <div className="space-y-4">
            {(data?.topSessionReaders || []).slice(0, 5).map((r: any, i: number) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <div className="text-end">
                  ...
                  <p className="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">✓ {r.completed_sessions} {t.admin.reportsPage.done}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-gray-800">{t.admin.reportsPage.mostActiveStudents}</h3>
          </div>
          <div className="space-y-4">
            {(data?.topStudents || []).slice(0, 5).map((s: any, i: number) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <div className="text-end">
                  <p className="text-xs text-gray-600 font-bold">{s.recitations} {t.admin.reportsPage.recs}</p>
                  <p className="text-[10px] text-gray-400">{s.bookings} {t.admin.reportsPage.sessionsCount}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex items-center gap-2">
          ...
          <h3 className="font-bold text-gray-800">{t.admin.reportsPage.overallTopContributors}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/50 text-gray-400 text-[11px] uppercase tracking-widest font-bold">
                <th className="py-3 px-6 text-start">{t.admin.reportsPage.rank}</th>
                <th className="py-3 px-6 text-start">{t.admin.reportsPage.reader}</th>
                <th className="py-3 px-6 text-start">{t.admin.reportsPage.reviews}</th>
                <th className="py-3 px-6 text-start">{t.admin.reportsPage.sessionsLabel}</th>
                <th className="py-3 px-6 text-start">{t.admin.reportsPage.total}</th>
              </tr>
            </thead>
